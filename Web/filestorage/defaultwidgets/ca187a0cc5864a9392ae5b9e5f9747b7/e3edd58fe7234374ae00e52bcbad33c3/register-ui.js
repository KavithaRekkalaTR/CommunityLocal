(function ($, global) {
	...
	_attachHandlers = function (context) {
		...
		$save.evolutionValidation('addField', context.inputs.email, { email: true, required: true, messages: { email: context.resources.emailFormatMessage, required: context.resources.emailRequiredMessage } }, '.field-item.email-address .field-item-validation', null);
		$save.evolutionValidation('addField', context.inputs.firstName, { required: true, messages: { required: context.resources.firstNameRequiredMessage } }, '.field-item.first-name .field-item-validation', null);
		$save.evolutionValidation('addField', context.inputs.lastName, { required: true, messages: { required: context.resources.lastNameRequiredMessage } }, '.field-item.last-name .field-item-validation', null)
		...
		$update.evolutionValidation('addField', context.inputs.email, { email: true, required: true, messages: { email: context.resources.emailFormatMessage, required: context.resources.emailRequiredMessage } }, '.field-item.email-address .field-item-validation', null);
		$update.evolutionValidation('addField', context.inputs.firstName, { required: true, messages: { required: context.resources.firstNameRequiredMessage } }, '.field-item.first-name .field-item-validation', null);
		$update.evolutionValidation('addField', context.inputs.lastName, { required: true, messages: { required: context.resources.lastNameRequiredMessage } }, '.field-item.last-name .field-item-validation', null);
		...
		$.telligent.evolution.post({
			...
			success: function (response) {
				if (response.redirectUrl) {
					window.parent.$.telligent.evolution.notifications.show(context.resources.registrationSuccessMessage);
					$.glowModal.close('return value');
				}
				else if (response.errors) {
					$.telligent.evolution.notifications.show(context.resources.saveErrorMessage + ' ' + response.errors[0].error, { type: 'warning' });
	...
	_cancelRegistrationWithCode = function (context) {
		...
		$.telligent.evolution.post({
			...
			success: function (response) {
				if (response.redirectUrl) {
					window.parent.$.telligent.evolution.notifications.show(context.resources.cancelSuccessMessage);
					$.glowModal.close('return value');
				}
				else if (response.errors) {
					alert(context.resources.cancelErrorMessage + '\n\n' + response.errors[0].error);
