(function ($) {
    ...
    function byTypeGraphClick(context, value, contentTypeId, contentTypeName) {
        context.localFilters.filterContentType = contentTypeId;
        context.localFilters.filterContentTypeName = contentTypeName;
        var start, end;
        ...
        if (context.localFilters.chartType == 'combined') {
            ...
            text = context.resources.filterByType.replace("{0}", context.localFilters.filterContentTypeName);
        }
    ...
    function resetFilter(context, deselect) {
        ...
        context.contentTypes = undefined;
    ...
    function hideSeries(context) {
        setEnabledContentTypes(context);

        if(context.localFilters.filterContentType != undefined && $.inArray(context.localFilters.filterContentType, context.contentTypes) < 0) {
            resetFilter(context, true);
        }
            ...
            context.fields.graphContainer.fadeTo(fadeDuration, 0.25, function () {
                $.telligent.evolution.post({ timeout: 90000,
                    ...
                    success: function (response) {
                        ...
                        var options = $.telligent.evolution.widgets.reporting.reportutil.getStandardGraphOptions(context, '#3F729B', 'Active Content', 'Number of active content', stringdata, resetFilter, graphClick);
        ...
        context.fields.graphContainer.fadeTo(fadeDuration, 0.25, function () {
            $.telligent.evolution.post({ timeout: 90000,
                ...
                success: function (response) {
                    ...
                    var options = $.telligent.evolution.widgets.reporting.reportutil.getMultiLineGraphOptions(context, 'Active Content', 'Number of active content', byTypeGraphClick, resetFilter, showSeries, hideSeries);
    ...
    function cleanData(context, tabData) {
        var selectedContent = tabData.selectedContent !== undefined ? tabData.selectedContent : [];
        if (selectedContent.length == 1) {
            tabData.selectedContent = undefined;
    ...
    $.telligent.reporting.widgets.activeContentReport = api;
})(jQuery);