(function ($) {
    ...
    function setVisibility(context) {
        if (context.filters.selectedApplications != undefined && context.filters.selectedApplications.length > 0) {
            ...
            showError(context, context.resources.noKnowledgeCollectionError);
        }
    };
            ...
            $.telligent.evolution.get({
                ...
                success: function (response) {
                    ...
                    if (r.find('.error').length > 0) {
                        ...
                        var knowledgecollectionDataContainer = $('.knowledgecollection-data', context.fields.reportContainer);
                        var knowledgecollectionData = $('.knowledgecollection-data', r);
                        if (knowledgecollectionData.length > 0 && knowledgecollectionDataContainer.length > 0) {
                            knowledgecollectionDataContainer.replaceWith(knowledgecollectionData);
                        }
    ...
    $.telligent.reporting.widgets.knowledgeCollectionSummaryReport = api;
})(jQuery);