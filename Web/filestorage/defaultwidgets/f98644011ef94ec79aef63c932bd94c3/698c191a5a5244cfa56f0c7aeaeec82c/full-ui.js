(function ($) {
    ...
    function byTypeGraphClick(context, value, activityTypeId, activityTypeName) {
        context.localFilters.filterActivityType = activityTypeId;
        context.localFilters.filterActivityTypeName = activityTypeName;
        var start, end;
        ...
        var text = '';
        if (context.localFilters.chartType == 'combined') {
            text = context.resources.filter;
        }
        else {
            text = context.resources.filterByType.replace("{0}", context.localFilters.filterActivityTypeName);
        }
    ...
    function hideSeries(context) {
        setEnabledActivityTypes(context);

        if(context.localFilters.filterActivityType != undefined && $.inArray(context.localFilters.filterActivityType, context.activityTypes) < 0) {
            resetFilter(context, true);
        }
    ...
    function resetFilter(context, deselect) {
        ...
        context.localFilters.filterActivityType = undefined;
        context.localFilters.filterActivityTypeName = undefined;
        context.activityTypes = undefined;
    ...
    function refreshGraph(context, fade) {
        ...
        if (context.localFilters.chartType == 'line' || context.localFilters.chartType == 'column') {
            byTypeRefreshGraph(context, fade);
        }
        else {
            ...
            context.fields.graphContainer.fadeTo(fadeDuration, 0.25, function () {
                $.telligent.evolution.post({ timeout: 90000,
                    ...
                    success: function (response) {
                        ...
                        var options = $.telligent.evolution.widgets.reporting.reportutil.getStandardGraphOptions(context, '#ff7c4c', 'Interactions', 'Number of interactions', stringdata, resetFilter, graphClick);
    ...
    function byTypeRefreshGraph(context, fade) {
        var fadeDuration = 0;
        if (fade === true) {
            fadeDuration = 400;
        }
        ...
        if (context.activityTypes) {
            data.fr_activityTypes = context.activityTypes.join(',');
        }

        context.fields.graphContainer.fadeTo(fadeDuration, 0.25, function () {
            $.telligent.evolution.post({ timeout: 90000,
                url: context.urls.byTypeReportData,
                data: data,
                success: function (response) {
                    context.fields.graphContainer.html(response);
                    context.fields.graphContainer.fadeTo(fadeDuration, 1);
                    ...
                    var options = $.telligent.evolution.widgets.reporting.reportutil.getMultiLineGraphOptions(context, 'Interactions', 'Number of interactions', byTypeGraphClick, resetFilter, showSeries, hideSeries);
                    $.telligent.evolution.widgets.reporting.reportutil.buildMultiLineSeriesData(context, options, graphdata, $.telligent.evolution.widgets.reporting.util.getActivityTypeSeriesColor);
    ...
    function handleGraphUpdateMessages(context, tabNameSpace) {
        $.telligent.evolution.messaging.subscribe('reporting.graph.update', tabNameSpace, function(data){
            ...
            if (type == 'line' || type =='column') {
                if (context.localFilters.chartType != 'line' && context.localFilters.chartType != 'column') {
                    ...
                    byTypeRefreshGraph(context, true);
                    resetFilter(context, true);
                }
                else {
                    context.localFilters.chartType = type;
    ...
    var api = {
        register: function (context) {
            ...
            handleGraphUpdateMessages(context, context.tabNameSpace);
        }
    };
    ...
    $.telligent.reporting.widgets.interactionsbyTypeReport = api;
})(jQuery);