		...
		return {
			prompt: function(importResult, onSelect) {
				...
				content.on('change', 'input[type="checkbox"]', function(e) {
					...
					if(request.model == 'embeddable') {
						var relatedImportableItem = content.find('input.importable-embeddable[data-embeddableid="' + cb.data('id') + '"]');
						if (cb.prop('checked')) {
				...
				content.on('click','a.continue', function(e){
					...
					var importCommands = [];
					content.find('input.importable-embeddable').each(function(){
				...
				var modal = $.glowModal({
					title: (importResult.resourceOnly ? context.resources.resourceImport : context.resources.embeddableImport),
					html: content,
					width: 550,
					height: '100%'
				});

				initMultiSelector(context, content.find('.field-item.new-embeddables'));
				initMultiSelector(context, content.find('.field-item.updated-embeddables'));
				initMultiSelector(context, content.find('.field-item.new-configured-embeddables'));
				initMultiSelector(context, content.find('.field-item.updated-configured-embeddables'));
			}
		}
	};
