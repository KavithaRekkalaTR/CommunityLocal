...
define('GlobalSearch', ['StudioStorageProxy'], function(StudioStorageProxy, $, global, undef) {
	...
	var scrollEndMessage = 'embeddable.search.scrollend';
	...
	function initUi(context) {
		...
		context.currentEmbeddableSelectOption = context.stateSelect.find('option[value="current"]');
		...
		removeCurrentEmbeddableOption(context);
	}

	function removeCurrentEmbeddableOption(context) {
		context.currentEmbeddableSelectOption.remove();
	}

	function addCurrentEmbeddableOption(context) {
		context.currentEmbeddableSelectOption.prependTo(context.stateSelect);
	}
	...
	function runSearch(context) {
		...
		// if "current embeddable" selected, scope to that embeddable's id
		if (stateSelectVal == 'current' && context.currentEmbeddable) {
			queryOptions.id = context.currentEmbeddable.id;
	...
	GlobalSearch.prototype.setCurrentEmbeddable = function(embeddableState) {
		if(embeddableState && embeddableState.id) {
			this.context.currentEmbeddable = embeddableState;
			addCurrentEmbeddableOption(this.context);
		} else {
			this.context.currentEmbeddable = null;
			removeCurrentEmbeddableOption(this.context);
		}
	};
