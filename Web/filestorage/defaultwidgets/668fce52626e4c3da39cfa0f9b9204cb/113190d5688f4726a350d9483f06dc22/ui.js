(function ($, global) {
	...
	var permissionsListApi = {
		register: function (options) {
			options.permissionHeaderWrapper = $('<div></div>');

			$.telligent.evolution.administration.header(options.permissionHeaderWrapper);

			options.wrapper = $.telligent.evolution.administration.panelWrapper();

			var headingTemplate = $.telligent.evolution.template.compile(options.permissionHeaderTemplateId);
			options.permissionHeaderWrapper.html(headingTemplate());
			$.telligent.evolution.administration.header();
            ...
            $('input[type="checkbox"][name="permissions"]', options.wrapper).on('change', function () {
                ...
                if ($(this).data('value') != this.checked) {
			...
			options.permissionHeaderWrapper.on('click', 'a.save-permissions', function() {
			...
			options.permissionHeaderWrapper.on('click', 'a.save-overwrite-permissions', function() {
	...
	$.telligent.evolution.widgets.groupDefaultPermissions = api;
	$.telligent.evolution.widgets.groupDefaultPermissionsRolesList = rolesListApi;
	$.telligent.evolution.widgets.groupDefaultPermissionsPermissionsList = permissionsListApi;
	$.telligent.evolution.widgets.groupDefaultPermissionsOverwritePermission = overwritePermissionApi;
