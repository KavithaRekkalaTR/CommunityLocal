(function($, global, undef) {
	...
	function collectSelections(context) {
		return context.wrapper
			.find('input.importable-embeddable')
			.toArray()
			.map(function(i) {
				var data = $(i);
				return {
	...
	function performImport(context, selection) {
		return $.telligent.evolution.embeddables.embeddableAdministration.loadWithProgress($.telligent.evolution.post({
	...
	var api = {
		register: function(options) {
			...
			var messageHandlers = {
				'embeddable-import': function(data) {
					...
					performImport(context, collectSelections(context)).then(function(){
						...
						$.telligent.evolution.messaging.publish('embeddable-imported');
						$.telligent.evolution.notifications.show(context.importCompleteText);
					});
				},
				'embeddable-import-cancel': function(data) {
			...
			initMultiSelector(context, context.wrapper.find('.field-item.new-embeddables'));
			initMultiSelector(context, context.wrapper.find('.field-item.updated-embeddables'));
		}
	}

	$.telligent.evolution.embeddables.embeddableAdministration.import = api;
