(function($, global, undef) {
	...
	function serializeImportCommands(commands) {
		...
		for(var i = 0; i < (commands || []).length; i++) {
			...
			if(commands[i].model && commands[i].model == 'configuration') {
				...
				serializedParts.push('`');
				serializedParts.push('emailconfiguration');
			} else {
				serializedParts.push('1');
			}
		}
	...
	function performImport(context, selection) {
		return $.telligent.evolution.widgets.scriptedEmailPlugin.loadWithProgress($.telligent.evolution.post({
	...
	var api = {
		register: function (options) {
			...
			context.wrapper.on('change', 'input[type="checkbox"]', function (e) {
				...
				var cb = $(e.target);
				if (cb.data('model') == 'email') {
					var relatedImportableItem = context.wrapper.find('input.importable-email[data-emailid="' + cb.data('id') + '"]');
					if (cb.prop('checked')) {
			...
			initMultiSelector(context, context.wrapper.find('.field-item.updated-emails'));
			initMultiSelector(context, context.wrapper.find('.field-item.updated-configured-emails'));
		}
	};

	$.telligent.evolution.widgets.scriptedEmailPlugin.import = api;
