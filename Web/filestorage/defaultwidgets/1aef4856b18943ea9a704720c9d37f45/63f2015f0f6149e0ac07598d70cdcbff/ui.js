(function($, global, undef) {
    ...
    function save(context) {
		...
		if (context.hasOptions && $.fn.dynamicForm) {
		    formOptions = $.telligent.evolution.url.serializeQuery($('#' + context.optionsId).dynamicForm('getValues'));
		}

		button.addClass('disabled');
		$.telligent.evolution.post({
		    url: $.telligent.evolution.site.getBaseUrl() + 'api.ashx/v2/contentconversion/conversion.json',
		    data: {
		        SourceContentTypeId: context.sourceContentTypeId,
		        SourceContentId: context.sourceContentId,
		        TargetContentTypeId: context.nodes.contentType.val(),
		        TargetApplicationId: targetApplication.applicationId,
		        TargetApplicationTypeId: targetApplication.applicationTypeId,
		        Options: formOptions
		    }
		})
		    .then(function(response) {
		        window.location = response.ContentConversion.Url;
		    })
		    .always(function() {
		        validateSave(context);
		    });
	...
	$.telligent.evolution.widgets.convertContentPanel = api;
