(function($, global) {
	...
	$.telligent.evolution.widgets.splitForumThread = {
		register: function(context) {

			var splitButton = $('<a href="#"></a>')
				.addClass('button split')
			...
			splitButton.evolutionValidation({
				validateOnLoad: false,
				onValidated: function(isValid, buttonClicked, c) {
				},
				onSuccessfulClick: function(e) {
					if (!splitButton.hasClass('disabled')) {
						splitButton.addClass('disabled');

						$.telligent.evolution.post({
							url: context.splitUrl,
							data: {
								ReplyId: context.replyId,
								ForumId: context.forumSelector.glowLookUpTextBox('getByIndex', 0).Value,
			...
			var validateLookup = splitButton.evolutionValidation(
				'addCustomValidation',
				'requiredLookup',
				function () {
						return context.forumSelector.glowLookUpTextBox('count') > 0;
				},
				context.text.required,
				'#' + context.forumSelectorWrapperId + ' .field-item-validation',
				null);

			context.forumSelector.glowLookUpTextBox({
					delimiter: ',',
