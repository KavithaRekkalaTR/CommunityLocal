		...
		register: function (options) {
			...
			if (options.canCreate) {
				...
				$('a.add-leaderboard').on('click', function(){
					$.telligent.evolution.administration.open({
						...
						cssClass: 'contextual-leaderboard-edit'
					});
			...
			$.telligent.evolution.messaging.subscribe('contextual.leaderboard.delete', function(data){
				if (confirm(options.resources.confirmDelete)) {
					...
					.then(function() {
						$.telligent.evolution.notifications.show(options.resources.leaderboardDeleted);
						...
						if (options.currentLeaderboardId == id) {
	...
	var editApi = {
		register: function (options) {
			...
			button.evolutionValidation({
				...
				onSuccessfulClick: function(e) {
					...
					.then(function(response) {
						...
						//if we are viewing the leaderboard and its no longer enabled redirect to group
						if (options.leaderboardId > 0 && options.leaderboardId == response.Id && response.Enabled != 'True') {
							window.location.href = options.urls.groupRedirect;
						}
						//if we are viewing the leaderboards and the application key changed redirect to new url
						else if(options.leaderboardId > 0 && options.leaderboardId == response.Id && options.applicationKey != appKey)
						{
			...
			.evolutionValidation('addCustomValidation', options.inputs.endDateId, function () {
				...
				, options.resources.invalidDateRange
				, '.field-item.time-enddate .field-item-validation', null);
			...
			button.evolutionValidation('addCustomValidation', options.inputs.timeSpanLengthId, function () {
				...
				, options.resources.fieldRequired
				, '.field-item.rolling-time .field-item-validation', null);
