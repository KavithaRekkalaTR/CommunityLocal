		...
		return {
			prompt: function(importResult, onSelect) {
				...
				content.on('click','a.continue', function(e){
					...
					var importCommands = [];
					content.find('input.importable-fragment').each(function(){
				...
				var modal = $.glowModal({
					title: (importResult.resourceOnly ? context.resources.resourceImport : context.resources.widgetImport),
					html: content,
					width: 550,
					height: '100%'
				});

				initMultiSelector(context, content.find('.field-item.new-fragments'));
				initMultiSelector(context, content.find('.field-item.updated-fragments'));
			}
		}
	};
