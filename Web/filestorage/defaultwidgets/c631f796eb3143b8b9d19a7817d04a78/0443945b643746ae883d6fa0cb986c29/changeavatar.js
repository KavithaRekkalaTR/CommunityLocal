(function($, global) {
	...
	if (typeof $.telligent.evolution.widgets === 'undefined') { $.telligent.evolution.widgets = {}; }
		...
		function validate(context) {
				...
				if (v == 'default') {
						...
						return selectedOption.closest('.field-item').find('.selectable-avatar.selected').length > 0;
				} else if (v == 'upload') {
						return !context.isUploading && context.file != null && context.file.fileName != null;
				} else if (v == 'link') {
						return context.fields.url.val().length > 0;
				} else {
						return false;
				}
		}
	...
	$.telligent.evolution.widgets.userAdministrationChangeAvatar = {
		register: function(context) {
			...
			context.saveButton = context.headerWrapper.find('.changeavatar');
					...
					if (!context.saveButton.hasClass('disabled')) {
							...
							var selectedOption = $('input[type="radio"]:checked', context.fields.options);
										...
										if (v == 'default') {
												...
												data.Change = 'url';
												data.Url = selectedOption.closest('.field-item').find('.selectable-avatar.selected').data('url');
										} else if (v == 'upload') {
												data.Change = 'file';
												...
												data.Change = 'url';
												data.Url = context.fields.url.val();
										}
										...
										if (data.Change) {
												$.telligent.evolution.post({
														...
														.then(function() {
																$.telligent.evolution.notifications.show(context.text.changeAvatarSuccessful, {
																		type: 'success'
																});
			...
			$('input[type="radio"]', context.fields.options).on('change input', function() {
					var selectedOption = $('input[type="radio"]:checked', context.fields.options);
						...
						$('.selectable-avatar', context.wrapper).on('click', function() {
							 var item = $(this);
							 if (!item.hasClass('selected')) {
									 $('.selectable-avatar.selected', context.wrapper).removeClass('selected');
			...
			context.fields.urlPreview = context.fields.url.closest('.field-item').find('.preview');
						context.fields.url.on('input', function() {
								clearTimeout(context.urlPreviewTimeout);
				context.urlPreviewTimeout = setTimeout(function () {
					$.telligent.evolution.post({
						url: context.urls.preview,
						data: {
								w_url: context.fields.url.val()
						},
						success: function (response) {
							response = $.trim(response);
							if (response && response.length > 0) {
									if (response !== context.urlPreviewContent) {
									context.urlPreviewContent = response;
									context.fields.urlPreview.html(context.urlPreviewContent).removeClass('empty');
								}
							} else {
									context.fields.urlPreview.empty().addClass('empty');
							}
						},
						error: function() {
								context.fields.urlPreview.empty().addClass('empty');
