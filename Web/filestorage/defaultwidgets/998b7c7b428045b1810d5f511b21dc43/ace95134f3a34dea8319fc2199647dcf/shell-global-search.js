...
define('GlobalSearch', ['StudioStorageProxy'], function(StudioStorageProxy, $, global, undef) {
	...
	var scrollEndMessage = 'email.search.scrollend';
	...
	function initUi(context) {
		...
		context.currentEmailSelectOption = context.stateSelect.find('option[value="current"]');
		...
		removeCurrentEmailOption(context);
	}

	function removeCurrentEmailOption(context) {
		context.currentEmailSelectOption.remove();
	}

	function addCurrentEmailOption(context) {
		context.currentEmailSelectOption.prependTo(context.stateSelect);
	}
	...
	function runSearch(context) {
		...
		// if "current email" selected, scope to that email's id
		if (stateSelectVal == 'current' && context.currentEmail) {
			queryOptions.id = context.currentEmail.id;
	...
	GlobalSearch.prototype.setCurrentEmail = function(emailState) {
		if(emailState && emailState.id) {
			this.context.currentEmail = emailState;
			addCurrentEmailOption(this.context);
		} else {
			this.context.currentEmail = null;
			removeCurrentEmailOption(this.context);
		}
	};
