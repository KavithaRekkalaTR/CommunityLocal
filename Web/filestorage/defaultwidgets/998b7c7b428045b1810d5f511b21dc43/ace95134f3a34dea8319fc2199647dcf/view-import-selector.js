			...
			prompt: function (importResult, onSelect) {
				...
				content.on('change', 'input[type="checkbox"]', function (e) {
					...
					if(request.model == 'email') {
						var relatedImportableItem = content.find('input.importable-email[data-emailid="' + cb.data('id') + '"]');
						if (cb.prop('checked')) {
				...
				content.on('click', 'a.continue', function (e) {
					...
					content.find('input.importable-email').each(function () {
				...
				var modal = $.glowModal({
					title: (importResult.resourceOnly ? context.resources.resourceImport : context.resources.emailImport),
					html: content,
					width: 550,
					height: '100%'
				});

				initMultiSelector(context, content.find('.field-item.updated-emails'));
				initMultiSelector(context, content.find('.field-item.updated-configured-emails'));
			}
		}
	};
