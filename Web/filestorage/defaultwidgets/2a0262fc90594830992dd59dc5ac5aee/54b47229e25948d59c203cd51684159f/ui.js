(function ($, global) {
				...
				onSuccessfulClick: function(e) {
					...
					if(continueSave)
					{
						...
						.then(function(response) {
							$.telligent.evolution.notifications.show(options.resources.wikiUpdated);
				...
				deleteButton.on('click', function () {
					if (window.confirm(options.resources.deleteConfirmation)) {
						$.telligent.evolution.del({
							url: $.telligent.evolution.site.getBaseUrl() + 'api.ashx/v2/wikis/' + options.wikiId + '.json',
							success: function (response) {
								window.location.href = options.urls.groupRedirect;
							}
						});
	...
	$.telligent.evolution.widgets.wikiOptionsManagement = api;
